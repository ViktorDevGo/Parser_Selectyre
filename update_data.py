#!/usr/bin/env python3
"""
Скрипт для автоматического обновления данных Selectyre (версия 2)
Логика:
...
def ensure_updated_at_column(conn):
    """Убедиться что колонка updated_at существует"""
    try:
        with conn.cursor() as cur:
            ...
            """)
            conn.commit()
        logger.info("✓ Поле updated_at проверено/добавлено")
    except Exception as e:
        logger.warning(f"⚠ Предупреждение при проверке updated_at: {e}")
        conn.rollback()
...
def update_table(conn):
    ...
    try:
        ...
        # Загрузка данных с UPSERT логикой
        with open(CSV_FILE_NEW, 'r', encoding=ENCODING) as csvfile:
            reader = csv.DictReader(csvfile, delimiter=DELIMITER)

            # UPSERT запрос: если ID существует - обновляем, если нет - вставляем
            upsert_query = """
                ...
                ON CONFLICT (id) DO UPDATE SET
                    ...
                    run_on_flat = EXCLUDED.run_on_flat,
                    homologation = EXCLUDED.homologation,
                    rim_protection = EXCLUDED.rim_protection,
                    sidewall_text = EXCLUDED.sidewall_text,
                    ...
                    axle = EXCLUDED.axle,
                    layering = EXCLUDED.layering,
                    studdable = EXCLUDED.studdable,
...
def get_update_statistics(conn, before_count):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Количество обновленных сегодня
            cur.execute("""
                SELECT COUNT(*)
                FROM Selectyre_tyer
            ...
            # Количество созданных сегодня (новых)
            cur.execute("""
                SELECT COUNT(*)
                FROM Selectyre_tyer
...
def main():
    ...
    logger.info("=" * 80)
    logger.info(f"НАЧАЛО ОБНОВЛЕНИЯ ДАННЫХ SELECTYRE (v2 - UPSERT)")
    ...
    try:
        ...
        # Проверка/добавление поля updated_at
